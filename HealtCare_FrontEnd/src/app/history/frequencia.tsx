...
export default function HeartRateHistoryScreen() {
  ...
  const applyFilter = () => {
    ...
    const parseDate = (v:string,end:boolean=false): number => { const m=/^(\d{2})\/(\d{2})\/(\d{4})$/.exec(v.trim()); if(!m) return NaN; const [,dd,mm,yyyy]=m; const h=end?23:0, mi=end?59:0, s=end?59:0; return new Date(parseInt(yyyy), parseInt(mm)-1, parseInt(dd), h, mi, s).getTime(); };
