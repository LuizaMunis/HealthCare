...
const HistoryItem: React.FC<{
  item: TemperatureHistoryItem;
  onEdit: (item: TemperatureHistoryItem) => void;
  onDelete: (id: number) => void;
}> = ({ item, onEdit, onDelete }) => {
  const formattedDate = new Date(item.data_hora_medicao).toLocaleString('pt-BR', {
  ...
  return (
    <View style={styles.itemContainer}>
      <View style={styles.itemDetails}>
        ...
        <Text style={styles.itemValues}>Temperatura: <Text style={styles.boldText}>{item.graus_celsius.toFixed(1)}</Text> °C</Text>
      </View>
...
export default function TemperatureHistoryScreen() {
  ...
  const handleUpdate = async () => {
    ...
    if (isNaN(tempNum) || tempNum < 30 || tempNum > 45) {
      Alert.alert('Atenção', 'Informe uma temperatura entre 30 e 45 °C');
      return;
    }
    try {
      const token = await AsyncStorage.getItem('healthcare_auth_token');
    ...
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      {records.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Feather name="thermometer" size={64} color="#CCCCCC" />
          <Text style={styles.emptyText}>Nenhum registro de temperatura</Text>
          ...
          >
            ...
            <Text style={styles.addButtonText}>Adicionar Temperatura</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <FlatList
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.formContainer}>
              ...
              <View style={styles.tempRow}>
                <TouchableOpacity style={styles.controlButton} onPress={() => setTemperature((prev) => {
                  const n = parseFloat(prev.replace(',','.')) || 0; return (n - 0.1).toFixed(1);
                })}><Feather name="minus-circle" size={32} color="#6c757d" /></TouchableOpacity>
                <Text style={styles.tempValue}>{temperature}</Text>
                <TouchableOpacity style={styles.controlButton} onPress={() => setTemperature((prev) => {
                  const n = parseFloat(prev.replace(',','.')) || 0; return (n + 0.1).toFixed(1);
                })}><Feather name="plus-circle" size={32} color="#6c757d" /></TouchableOpacity>
              </View>
