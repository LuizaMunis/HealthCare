...
const TemperatureChart: React.FC<{ data: TemperatureHistoryItem[] }> = ({ data }) => {
  const [svgLib, setSvgLib] = React.useState<any>(null);
  const [selectedPoint, setSelectedPoint] = React.useState<{ x:number; y:number; item:TemperatureHistoryItem } | null>(null);

  React.useEffect(()=>{
    let mounted = true;
    import('react-native-svg').then(mod=> mounted && setSvgLib(mod)).catch(()=>{});
  ...
  const tempValues = sorted.map(r => r.graus_celsius);
  const min = Math.min(...tempValues) - 0.5;
  const max = Math.max(...tempValues) + 0.5;
  const chartHeight = 160;
  const chartWidth = Math.max(360, sorted.length * 70);
  const valueToY = (v:number) => {
    const range = max - min || 1;
    return chartHeight - ((v - min) / range) * (chartHeight - 20) - 10;
  };
  const xForIndex = (i:number) => (i/(sorted.length-1||1))*(chartWidth-40)+20;
  const points = tempValues.map((v,i)=> `${xForIndex(i)},${valueToY(v)}`).join(' ');

  const renderTooltip = () => {
    if (!selectedPoint) return null;
    const { x, y, item } = selectedPoint;
    const date = formatDateTime(item.data_hora_medicao);
    const adjustedX = Math.min(Math.max(x - 60, 4), chartWidth - 140);
    const adjustedY = y < 60 ? y + 20 : y - 55;
    return (
      <View style={[styles.tooltipContainer,{ left:adjustedX, top:adjustedY }]}> 
        <Text style={styles.tooltipTitle}>{date}</Text>
        <Text style={styles.tooltipValue}>Temp: {item.graus_celsius.toFixed(1)} °C</Text>
      </View>
    );
  };
  ...
  return (
    <View style={styles.chartCard}>
      <Text style={styles.sectionTitle}>Evolução da Temperatura</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View style={styles.chartInnerWrapper}>
          <Svg width={chartWidth} height={chartHeight}>
            ...
            <Polyline points={points} fill="none" stroke="#FF9800" strokeWidth={3} />
            {tempValues.map((v,i)=> (
              <Circle key={`t-${i}`} cx={xForIndex(i)} cy={valueToY(v)} r={5} fill="#FF9800" onPress={()=> setSelectedPoint({ x:xForIndex(i), y:valueToY(v), item:sorted[i] })} />
            ))}
          </Svg>
      ...
      <View style={styles.legendRow}>
        <View style={styles.legendItem}><View style={[styles.legendDot,{backgroundColor:'#FF9800'}]} /><Text style={styles.legendLabel}>Temperatura</Text></View>
        <View style={styles.legendRangeBadge}><Text style={styles.legendRangeText}>{(min+0.5).toFixed(1)} - {(max-0.5).toFixed(1)} °C</Text></View>
      </View>
    </View>
  );
};
...
export default function TemperatureHistoryScreen() {
  ...
  const [records, setRecords] = useState<TemperatureHistoryItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [filteredRecords, setFilteredRecords] = useState<TemperatureHistoryItem[] | null>(null);

  const [isModalVisible, setIsModalVisible] = useState(false);
  const [editingRecord, setEditingRecord] = useState<TemperatureHistoryItem | null>(null);
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.TEMPERATURE_RECORDS}?perfil_id=${profileId}`, { headers:{ 'Authorization':`Bearer ${token}` } });
      const result = await response.json();
      if (!response.ok) throw new Error(result.message || 'Falha ao buscar o histórico.');
      const rows = Array.isArray(result.data) ? result.data : [];
      rows.sort((a:TemperatureHistoryItem,b:TemperatureHistoryItem)=> new Date(b.data_hora_medicao).getTime() - new Date(a.data_hora_medicao).getTime());
      setRecords(rows);
    ...
    Alert.alert('Confirmar Exclusão','Você tem certeza que deseja excluir este registro?',[
      ...
      { text:'Excluir', style:'destructive', onPress: async ()=> {
        try {
          ...
          const resp = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.TEMPERATURE_RECORDS}/${id}?perfil_id=${profileId}`,{ method:'DELETE', headers:{ 'Authorization':`Bearer ${token}` } });
  ...
  const handleEdit = (item:TemperatureHistoryItem) => {
    setEditingRecord(item);
    setTemperature(item.graus_celsius.toFixed(1));
  ...
  const handleUpdate = async () => {
    ...
    if (isNaN(tempNum) || tempNum < 30 || tempNum > 45) { Alert.alert('Atenção','Informe uma temperatura entre 30 e 45 °C'); return; }
    try {
      const token = await AsyncStorage.getItem('healthcare_auth_token');
      const profileId = await AsyncStorage.getItem('active_profile_id');
      if (!profileId) { Alert.alert('Erro','Nenhum perfil ativo encontrado.'); return; }
      const url = `${API_CONFIG.BASE_URL}${ENDPOINTS.TEMPERATURE_RECORDS}/${editingRecord.id}?perfil_id=${profileId}`;
      const time = new Date(editingRecord.data_hora_medicao).toTimeString().slice(0,8);
      const resp = await fetch(url, { method:'PUT', headers:{ 'Content-Type':'application/json', 'Authorization':`Bearer ${token}` }, body: JSON.stringify({ graus_celsius: tempNum, data_hora_medicao: `${date}T${time}`, perfil_id: parseInt(profileId) }) });
      const data = await resp.json();
      if (!resp.ok) throw new Error(data.message || 'Falha ao atualizar o registro.');
      Alert.alert('Sucesso!','Registro atualizado.');
      setIsModalVisible(false); setEditingRecord(null); fetchHistory();
  ...
  const applyFilter = () => {
    ...
    const parseDate = (val:string, end:boolean=false): number => {
      ...
      if (!m) return NaN; const [,dd,mm,yyyy] = m; const day = +dd; const month = +mm -1; const year = +yyyy; const h=end?23:0, mi=end?59:0, s=end?59:0; return new Date(year,month,day,h,mi,s).getTime();
    };
    ...
    const subset = records.filter(r=> { const t = new Date(r.data_hora_medicao).getTime(); return t>=start && t<=end; });
  ...
  if (error && records.length === 0) return (
    <View style={styles.centered}><Text style={styles.errorText}>Erro ao carregar dados: {error}</Text><TouchableOpacity onPress={fetchHistory} style={styles.retryButton}><Text style={styles.saveButtonText}>Tentar Novamente</Text></TouchableOpacity></View>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} nestedScrollEnabled>
        ...
        <TemperatureChart data={filteredRecords || records} />
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <View style={[styles.tableRow, item.id % 2 === 0 && styles.tableRowAlt]}>
                    ...
                    <Text style={styles.tableCell}>{item.graus_celsius.toFixed(1)}</Text>
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.formContainer}>
              ...
              <View style={styles.metricBody}>
                <TouchableOpacity style={styles.metricControl} onPress={()=> setTemperature(prev => { const n = parseFloat(prev.replace(',','.'))||0; return (n-0.1).toFixed(1); })}><Feather name="minus" size={22} color="#555" /></TouchableOpacity>
                <View style={styles.metricValueWrap}><Text style={styles.metricValue}>{temperature}</Text><Text style={styles.metricUnit}>°C</Text></View>
                <TouchableOpacity style={styles.metricControl} onPress={()=> setTemperature(prev => { const n = parseFloat(prev.replace(',','.'))||0; return (n+0.1).toFixed(1); })}><Feather name="plus" size={22} color="#555" /></TouchableOpacity>
              </View>
...
const styles = StyleSheet.create({
  ...
  tooltipContainer: { position:'absolute', padding:8, backgroundColor:'#004A61', borderRadius:8, minWidth:120 },
