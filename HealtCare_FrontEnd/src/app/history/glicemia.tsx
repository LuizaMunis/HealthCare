...
export default function GlycemiaHistoryScreen() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.GLYCEMIA_RECORDS}`, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleDelete = async (id: number) => {
    Alert.alert('Confirmar Exclusão', 'Você tem certeza que deseja excluir este registro?', [
      ...
      { text: 'Excluir', style: 'destructive', onPress: async () => {
          try {
            const token = await AsyncStorage.getItem('healthcare_auth_token');
            const response = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.GLYCEMIA_RECORDS}/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
            if (!response.ok) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('healthcare_auth_token');
      const url = `${API_CONFIG.BASE_URL}${ENDPOINTS.GLYCEMIA_RECORDS}/${editingRecord.id}`;
      const time = new Date(editingRecord.data_hora_medicao).toTimeString().slice(0,8);
      const response = await fetch(url, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ valor_mg_dl: v, data_hora_medicao: `${date}T${time}` })
      });
