...
const GlycemiaChart: React.FC<{ data: GlycemiaItem[] }> = ({ data }) => {
  const [svgLib, setSvgLib] = React.useState<any>(null);
  const [selectedPoint, setSelectedPoint] = React.useState<{ x:number; y:number; item:GlycemiaItem } | null>(null);
  ...
  const values = sorted.map(r=> r.valor_mg_dl);
...
const GlycemiaChart: React.FC<{ data: GlycemiaItem[] }> = ({ data }) => {
  ...
  return (
    <View style={styles.chartCard}>
      <Text style={styles.sectionTitle}>Evolução da Glicemia</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View style={styles.chartInnerWrapper}>
          <Svg width={chartWidth} height={chartHeight}>
            ...
            <Polyline points={points} fill="none" stroke="#7B1FA2" strokeWidth={3} />
            {values.map((v,i)=> <Circle key={`g-${i}`} cx={xForIndex(i)} cy={valueToY(v)} r={5} fill="#7B1FA2" onPress={()=> setSelectedPoint({ x:xForIndex(i), y:valueToY(v), item:sorted[i] })} />)}
          </Svg>
      ...
      <View style={styles.legendRow}>
        <View style={styles.legendItem}><View style={[styles.legendDot,{backgroundColor:'#7B1FA2'}]} /><Text style={styles.legendLabel}>Glicemia</Text></View>
        <View style={styles.legendRangeBadge}><Text style={styles.legendRangeText}>{min+5} - {max-5} mg/dL</Text></View>
    <View style={styles.chartCard}>
      <Text style={styles.sectionTitle}>Evolução da Glicemia</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View style={styles.chartInnerWrapper}>
...
export default function GlycemiaHistoryScreen() {
  ...
  const [filteredRecords, setFilteredRecords] = useState<GlycemiaItem[] | null>(null);
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [filteredRecords, setFilteredRecords] = useState<GlycemiaItem[] | null>(null);
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.GLYCEMIA_RECORDS}?perfil_id=${profileId}`, { headers:{ 'Authorization':`Bearer ${token}` } });
      const result = await resp.json();
      if (!resp.ok) throw new Error(result.message || 'Falha ao buscar o histórico.');
    ...
    Alert.alert('Confirmar Exclusão','Você tem certeza que deseja excluir este registro?',[
      ...
      { text:'Excluir', style:'destructive', onPress: async ()=> {
        try {
          ...
          const resp = await fetch(`${API_CONFIG.BASE_URL}${ENDPOINTS.GLYCEMIA_RECORDS}/${id}?perfil_id=${profileId}`, { method:'DELETE', headers:{ 'Authorization':`Bearer ${token}` } });
  ...
  const handleUpdate = async () => {
    ...
    if (isNaN(v) || v < 40 || v > 600) { Alert.alert('Atenção','Informe um valor entre 40 e 600 mg/dL'); return; }
    const v = parseInt(value,10);
    if (isNaN(v) || v < 40 || v > 600) { Alert.alert('Atenção','Informe um valor entre 40 e 600 mg/dL'); return; }
    try {
      const token = await AsyncStorage.getItem('healthcare_auth_token');
      ...
      const resp = await fetch(url, { method:'PUT', headers:{ 'Content-Type':'application/json','Authorization':`Bearer ${token}` }, body: JSON.stringify({ valor_mg_dl: v, data_hora_medicao: `${date}T${time}`, perfil_id: parseInt(profileId) }) });
  ...
  const applyFilter = () => {
    ...
    const parseDate = (v:string, end:boolean=false): number => { const m=/(\d{2})\/(\d{2})\/(\d{4})$/.exec(v.trim()); if(!m) return NaN; const [,dd,mm,yyyy]=m; const day=+dd; const month=+mm-1; const year=+yyyy; const h=end?23:0, mi=end?59:0, s=end?59:0; return new Date(year,month,day,h,mi,s).getTime(); };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} nestedScrollEnabled>
        ...
        <GlycemiaChart data={filteredRecords || records} />

        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Registros de Glicemia</Text>
          <View style={styles.tableHeader}>
            <Text style={[styles.tableHeaderCell,{flex:2}]}>Data</Text>
            <Text style={styles.tableHeaderCell}>Valor (mg/dL)</Text>
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <View style={[styles.tableRow, item.id % 2 === 0 && styles.tableRowAlt]}>
                    ...
                    <Text style={styles.tableCell}>{item.valor_mg_dl}</Text>
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.formContainer}>
              ...
              <View style={styles.metricBody}>
                <TouchableOpacity style={styles.metricControl} onPress={()=> setValue(prev => String(Math.max(40,(parseInt(prev||'0',10)-1))))}><Feather name="minus" size={22} color="#555" /></TouchableOpacity>
                <View style={styles.metricValueWrap}><Text style={styles.metricValue}>{value}</Text><Text style={styles.metricUnit}>mg/dL</Text></View>
                <TouchableOpacity style={styles.metricControl} onPress={()=> setValue(prev => String(Math.min(600,(parseInt(prev||'0',10)+1))))}><Feather name="plus" size={22} color="#555" /></TouchableOpacity>
              <Text style={styles.inputLabel}>Glicemia (mg/dL)</Text>
              <View style={styles.metricBody}>
                <TouchableOpacity style={styles.metricControl} onPress={()=> setValue(prev => String(Math.max(40,(parseInt(prev||'0',10)-1))))}><Feather name="minus" size={22} color="#555" /></TouchableOpacity>
                <View style={styles.metricValueWrap}><Text style={styles.metricValue}>{value}</Text><Text style={styles.metricUnit}>mg/dL</Text></View>
