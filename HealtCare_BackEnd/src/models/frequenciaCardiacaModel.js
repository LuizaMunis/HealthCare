...
class FrequenciaCardiacaModel {
  /**
   * Cria a tabela 'registro_frequencia_cardiaca' se ela não existir.
   */
  static async createTable() {
    const query = `
      CREATE TABLE IF NOT EXISTS registro_frequencia_cardiaca (
        id INT NOT NULL AUTO_INCREMENT,
        perfil_id INT NOT NULL,
        bpm INT NOT NULL,
        data_hora_medicao DATETIME NOT NULL,
        PRIMARY KEY (id),
        INDEX fk_registro_frequencia_cardiaca_perfil_idx (perfil_id ASC),
        CONSTRAINT fk_registro_frequencia_cardiaca_perfil
    ...
    try {
      ...
      console.log('✅ Tabela registro_frequencia_cardiaca criada/verificada com sucesso!');
    } catch (error) {
      console.error('❌ Erro ao criar tabela registro_frequencia_cardiaca:', error);
      throw error;
    }
  }
   ...
   * @param {object} dadosRegistro - Contém perfil_id, bpm, data_hora_medicao.
  ...
  static async create(dadosRegistro) {
    ...
    const query = `
      INSERT INTO registro_frequencia_cardiaca (perfil_id, bpm, data_hora_medicao)
      VALUES (?, ?, ?)
    `;
  ...
  static async findByPerfilId(perfilId) {
    const query = 'SELECT * FROM registro_frequencia_cardiaca WHERE perfil_id = ? ORDER BY data_hora_medicao DESC';
    try {
  ...
  static async findById(registroId) {
    const query = 'SELECT * FROM registro_frequencia_cardiaca WHERE id = ?';
    try {
  ...
  static async update(registroId, updateData) {
    ...
    const query = `UPDATE registro_frequencia_cardiaca SET ${fields} WHERE id = ?`;
    try {
      const [result] = await pool.execute(query, values);
  ...
  static async delete(registroId) {
    const query = 'DELETE FROM registro_frequencia_cardiaca WHERE id = ?';
    try {
